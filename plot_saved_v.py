...
def plotsaved(plotFrom, time,Wnt=True,Shh=True,Rostro=True,Dorso=True,Mix=True,savePic=False):
    ...
    if Mix == True:
        ...
        fig = plt.figure(figsize=figsize)
        axMix = fig.add_subplot(2,1,1,projection='3d')
        axMixDownside = fig.add_subplot(2,1,2,projection='3d')
        axMix.set_title("Gene combinations  t = %1.1f s" % time)
        ...
        axMix.legend([proxy0, proxy3, proxy6, proxy1, proxy4, proxy7, proxy2,proxy5,proxy8],['FB-P', 'FB-O', 'FB-N', 'MB-P', 'MB-O', 'MB-N', 'HB-P', 'HB-O','HB-N'],bbox_to_anchor=(1.12, 1), loc=2, borderaxespad=0.5)
        axMix.set_zorder(100)
        if savePic == True:
            if plotmode == 'Voxel':
                plt.savefig(mixDir +'pictures/'+'T%1.1f' %time + '_Mix_Vox.png')
                plt.savefig(allDir +'T%1.1f' %time + '_Mix_Vox.png')
            
            else:
                plt.savefig(mixDir +'pictures/'+'T%1.1f' %time + '_Mix.png')
                plt.savefig(allDir +'T%1.1f' %time + '_Mix.png')
            plt.close()
...
#plotTime(dataPath,500.0,dorso=True,rostro=False,mix=False)
#print("done")
#plt.show()
